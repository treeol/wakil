...
type App struct {
	...
	// costState, turnState, and subagentState are extracted App state groups
	// (WP-6.3). Embedded so promoted-field access (a.Costs, a.WorkflowStepTrace,
	// a.exhausted, ...) is unchanged — see cost_state.go, turn_state.go,
	// subagent_state.go.
	costState
	turnState
	subagentState
...
func (a *App) buildPreamble(today string) string {
	...
	// root and ancestor directories (bounded by the workspace root, never
	// walking above it). Inserted AFTER Wakil's own instructions and BEFORE
	// the date line so the precedence rule is visually obvious:
	...
	var wsRoot string
	if a.Exec != nil {
		wsRoot = a.Exec.WorkspaceRoot()
	}
	if agentsMD := loadAgentsMD(cwd, wsRoot); agentsMD != "" {
