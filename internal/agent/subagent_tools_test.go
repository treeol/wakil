...
func TestCapabilityToolsWithoutConsent(t *testing.T) {
	...
	if !strings.Contains(result, "ERROR: tools capability requires") {
...
func TestCapabilityValidationNamesTools(t *testing.T) {
	...
	if !strings.Contains(result, "tools") {
		t.Errorf("error should name 'tools' as a valid value, got: %s", result)
	}
}
...
func TestDiscoveryEditTiersUnchangedByToolsTier(t *testing.T) {
	...
	// Edit tier has exactly 22 tools (5 discovery + 4 edit + 5 staging + 8 memory).
	if len(e1) != 22 {
		t.Errorf("EditTools should have 22 tools, got %d", len(e1))
	}
	// Discovery tier has exactly 18 tools (5 read-only + 5 staging + 8 memory).
	if len(d1) != 18 {
		t.Errorf("DiscoveryTools should have 18 tools, got %d", len(d1))
	}
}
