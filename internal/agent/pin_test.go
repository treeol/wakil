...
func TestExhaustedSubagentReturnsIncompleteStatusRealPath(t *testing.T) {
	...
	summary, _, _, _ := parent.dispatchSubagent(context.Background(), "find things", io.Discard, "")
...
func TestExhaustionSurvivesRetryRealPath(t *testing.T) {
	...
	summary, _, _, _ := parent.dispatchSubagent(context.Background(), "find", io.Discard, "")
